...
HOSTNAME='borg-archy.dsolutionz.com'
...
partition_drive() {
    ...
    parted -s "$dev" \
        mklabel msdos \
        mkpart primary xfs 1 100M \
        mkpart primary xfs 250M 100% \
        set 1 boot on \
        set 1 boot on \
        set 2 LVM on
}
...
format_filesystems() {
    ...
    mkfs.xfs -f -L root /dev/root_vg/root
...
install_base() {
    ...
    pacstrap /mnt base base-devel linux linux-headers linux-firmware lvm2 grub networkmanager
}
...
/dev/root_vg/swap   none    swap   defaults            0   0
/dev/root_vg/root   /       xfs    defaults,relatime   0   1
/dev/sda1           /boot   xfs    defaults,relatime   0   2
EOF
}
...
TARBS_setup(){
    curl -LO raw.githubusercontent.com/tzurita/TARBS/master/archi3/tarbs.sh && bash tarbs.sh
}
